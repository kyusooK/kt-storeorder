<template>
    <v-card outlined @click="openDialog">
        <v-card-title>
            Stroe : {{ referenceValue ? referenceValue._links.self.href.split('/').pop() : '' }}
        </v-card-title>

        <v-dialog v-model="pickerDialog">
            <v-card>
                <v-card-title>Stroe</v-card-title>
                <v-card-text>
                    <StroePicker v-model="value" @selected="pick"/>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
        ...
        methods: {
            ...
            openDialog() {
                ...
                if(this.editMode) {
                    ...
                    this.$router.push(path + this.value.stroeId);
                }
            },
            async pick(val){
                this.newValue = val;
