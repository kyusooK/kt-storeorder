<template>
    <v-card outlined @click="openDialog">
        <v-card-title>
            Store : {{ referenceValue ? referenceValue._links.self.href.split('/').pop() : '' }}
        </v-card-title>
        ...
        <v-dialog v-model="pickerDialog" width="500">
            <v-card>
                ...
                <v-card-text>
                    <StorePicker v-model="value" @selected="pick"/>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
        ...
        methods: {
            ...
            openDialog() {
                ...
                if(this.editMode) {
                    ...
                    this.$router.push(path + this.value.storeId);
                }
            },
            async pick(val){
                this.newValue = val;
                var path = '/stores';
                var temp = await axios.get(axios.fixUrl(path + '/' + val.storeId));
                if(temp.data) {
                    this.referenceValue = temp.data;
                }
