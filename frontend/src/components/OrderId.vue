<template>
    <v-card outlined @click="openDialog">
        <v-card-title>
            Order : {{ referenceValue ? referenceValue._links.self.href.split('/').pop() : '' }}
        </v-card-title>
        ...
        methods: {
            ...
            openDialog() {
                ...
                if(this.editMode) {
                    ...
                    this.$router.push(path + this.value.orderId);
                }
            },
            async pick(val){
                this.newValue = val;
                var path = '/orders';
                var temp = await axios.get(axios.fixUrl(path + '/' + val.orderId));
                if(temp.data) {
                    this.referenceValue = temp.data;
                }
